...
const { data, isLoading, mutate } = useSWR(["studiesByCriteria", criteria],() => fetchGetStudiesByCriteria(),{
   ...
   async function fetchGetStudiesByCriteria(){
    if(!id) return;
    try{
      const response = await Axios.get<StudiesByCriteriaData>(path,getRequestOptions());
