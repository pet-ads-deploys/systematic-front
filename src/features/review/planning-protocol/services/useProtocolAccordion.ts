...
const useProtocolAccordion = () => {
  ...
  useEffect(() => {
    ...
    let url = `systematic-study/${id}/protocol`;
    ...
    Axios.get(url, options)
      .then((res) => {
        ...
        function isPicocInitialized() {
          return (
            (res.data.content.picoc.control &&
              res.data.content.picoc.control.trim() !== "") ||
            (res.data.content.picoc.intervention &&
              res.data.content.picoc.intervention.trim() !== "") ||
            (res.data.content.picoc.outcome &&
              res.data.content.picoc.outcome.trim() !== "") ||
            (res.data.content.picoc.population &&
              res.data.content.picoc.population.trim() !== "")
          );
        }

        function isPicocFinished() {
          return (
            res.data.content.picoc.control !== null &&
            res.data.content.picoc.intervention !== null &&
            res.data.content.picoc.outcome !== null &&
            res.data.content.picoc.population !== null
          );
        }
