...
interface AppContextType {
  ...
  setSortedExtractionStudyIndex: React.Dispatch<
    React.SetStateAction<number | undefined>
  >;
...
export const AppProvider: React.FC<AppProviderProps> = ({ children }) => {
  ...
  const [selectionStudy, setSelectionStudy] = useState<
    StudyInterface | undefined
  >(showFirstPossibleStudy(ExcutionFaseEnum.SELECTION));
  const [showExtractionModal, setShowExtractionModal] = useState(false);
  const [extractionStudy, setExtractionStudy] = useState<
    StudyInterface | undefined
  ...
  const [selectionStudyIndex, setSelectionStudyIndex] = useState<
    number | undefined
  >();
  const [sortedExtractionStudyIndex, setSortedExtractionStudyIndex] = useState<
    number | undefined
  >();
