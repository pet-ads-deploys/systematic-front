...
export default function StudySelectionArea({ type }: PageLayout) {
  ...
  if (setSelectionStudies && studyData?.articles)
    setSelectionStudies(studyData?.articles as StudyInterface[]);
  ...
  return (
    <StudySelectionProvider>
      ...
      >
        ...
        <Box w={"100%"} h="100%">
          {studyData?.articles && typeof studyIndex === "number" ? (
            <StudyDataFiel
              studyData={studyData?.articles?.[studyIndex] as StudyInterface}
              page={{ type }}
            />
          ) : null}
        </Box>
